...
export const checkOnboardingSettings = async (req, res, next) => {
  try {
    ...
    // User from authorizeSeller middleware
    const user = req.user;

    // 👉 Case 1: Demo access = true (Read-only access)
    if (user.demoAccess) {
      ...
      if (["POST", "PUT", "PATCH", "DELETE"].includes(method)) {
        return res.status(403).json({
          success: false,
          message:
            "Demo access only allows read-only operations. Please upgrade to a full account to modify data.",
        });
      }
    ...
    // 👉 Case 2: Normal/full account (no demo restrictions)
    return next();
  } catch (error) {
