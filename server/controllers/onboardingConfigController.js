...
export const updateOnboardingStep = async (req, res) => {
  try {
    ...
    if (description !== undefined)
      config.steps[stepIndex].description = description;
    if (isRequired !== undefined)
      config.steps[stepIndex].isRequired = isRequired;
    ...
    if (estimatedTimeMinutes !== undefined)
      config.steps[stepIndex].estimatedTimeMinutes = estimatedTimeMinutes;
...
export const updateDocumentVerificationSettings = async (req, res) => {
  try {
    ...
    if (isEnabled !== undefined)
      config.documentVerification.isEnabled = isEnabled;
    if (isRequired !== undefined)
      config.documentVerification.isRequired = isRequired;
    if (allowSkip !== undefined)
      config.documentVerification.allowSkip = allowSkip;
    if (skipMessage !== undefined)
      config.documentVerification.skipMessage = skipMessage;
    if (autoApproval !== undefined)
      config.documentVerification.autoApproval = autoApproval;
    if (approvalTimeoutHours !== undefined)
      config.documentVerification.approvalTimeoutHours = approvalTimeoutHours;
...
export const addDocumentType = async (req, res) => {
  try {
    ...
    const existingDoc = config.documentVerification.documentTypes.find(
      (doc) => doc.name === name
    );
    if (existingDoc) {
      return res.status(400).json({
...
export const updateDocumentType = async (req, res) => {
  try {
    ...
    const config = await OnboardingConfig.getConfig();
    const docIndex = config.documentVerification.documentTypes.findIndex(
    ...
    if (name !== undefined)
      config.documentVerification.documentTypes[docIndex].name = name;
    if (description !== undefined)
      config.documentVerification.documentTypes[docIndex].description =
        description;
    if (acceptedFormats !== undefined)
      config.documentVerification.documentTypes[docIndex].acceptedFormats =
        acceptedFormats;
    if (maxSizeInMB !== undefined)
      config.documentVerification.documentTypes[docIndex].maxSizeInMB =
        maxSizeInMB;
    if (isRequired !== undefined)
      config.documentVerification.documentTypes[docIndex].isRequired =
        isRequired;
    if (isActive !== undefined)
      config.documentVerification.documentTypes[docIndex].isActive = isActive;
    if (validationRules !== undefined) {
...
export const getAuditLogs = async (req, res) => {
  try {
    ...
    const config = await OnboardingConfig.getConfig().populate(
      "modificationHistory.modifiedBy",
      "names email"
    );
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        requiredSteps: config.steps.filter(
          (step) => step.isRequired && step.isActive
        ).length,
        optionalSteps: config.steps.filter(
          (step) => !step.isRequired && step.isActive
        ).length,
        stepStats,
      },
