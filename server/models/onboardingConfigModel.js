...
const onboardingConfigSchema = new mongoose.Schema(
  {
    ...
    documentVerification: {
      ...
      skipMessage: {
        type: String,
        default:
          "You can skip document verification and complete it later from your profile.",
      },
      ...
      steps: [
        ...
        },
        {
          ...
          validationRules: {
            skipAllowed: true,
            skipMessage:
              "Document verification can be completed later to unlock additional features",
            requiredFields: [],
          },
        },
      ],
...
onboardingConfigSchema.statics.updateConfig = async function (
  updates,
  userId,
...
onboardingConfigSchema.methods.getRequiredDocumentTypes = function () {
  return this.documentVerification.documentTypes.filter(
    (doc) => doc.isRequired && doc.isActive
  );
};

const OnboardingConfig = mongoose.model(
  "OnboardingConfig",
