...
import { FaSignOutAlt, FaRegClock, FaForward, FaCheck, FaExclamationTriangle } from "react-icons/fa";
...
const EnhancedSellerOnboarding = () => {
  ...
  useEffect(() => {
    const initializeOnboarding = async () => {
      try {
        ...
        const progressResponse = await axios.get('/api/sellers/onboarding-status');
      ...
      if (response.success) {
        ...
        if (currentStep < onboardingConfig.steps.length - 1) {
          ...
          if (allRequiredCompleted) {
            ...
            const nextRequiredStep = onboardingConfig.steps.findIndex(s => 
              s.isRequired && s.isActive && !newCompletedSteps.includes(s.stepId)
            );
  ...
  const submitShopTiming = async (data) => {
    ...
    const response = await axios.post(`/api/shop-timing/${sellerId}`, requestData);
    return response.data;
  };
  ...
  const renderStepContent = () => {
    ...
    switch (step.stepId) {
      case 'basic_details':
        return <BasicDetailsStep onSubmit={handleStepSubmit} formData={formData} setFormData={setFormData} categories={categories} brands={brands} />;
      case 'shop_timing':
        return <ShopTimingStep onSubmit={handleStepSubmit} formData={formData} setFormData={setFormData} />;
      case 'document_verification':
        return <DocumentVerificationStep onSubmit={handleStepSubmit} formData={formData} setFormData={setFormData} config={onboardingConfig} />;
      case 'subscription':
        return <SubscriptionStep onSubmit={handleStepSubmit} formData={formData} setFormData={setFormData} plans={plans} />;
      default:
  ...
  if (!onboardingConfig) {
    return (
      <div className="onboarding-container">
        <div className="error-message">Failed to load onboarding configuration</div>
      </div>
    );
  }
    ...
    <div className="onboarding-container">
      <div className="onboarding-form">
        ...
        <div className="onboarding-header">
          ...
          <h1 className="onboarding-main-heading">Welcome {getSellerName()}!</h1>
          <div className="onboarding-note">
            <span role="img" aria-label="info" style={{ marginRight: 8 }}>ℹ️</span>
            <span>
              Complete your seller onboarding to unlock your dashboard and start selling!
            </span>
          </div>
        </div>
          ...
          {currentStepData && (
            <div className="step-header">
              ...
              {currentStepData.category && (
                <span className="step-category">{currentStepData.category}</span>
              )}
            </div>
          )}
...
const BasicDetailsStep = ({ onSubmit, formData, setFormData, categories, brands }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="form-step">
      ...
      <div className="form-group">
        ...
        <small className="form-help">Hold Ctrl/Cmd to select multiple categories</small>
      </div>
      ...
      <div className="form-group">
        ...
        <small className="form-help">Hold Ctrl/Cmd to select multiple brands</small>
      </div>
      ...
      <div className="form-group">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            newAddresses[0] = { ...newAddresses[0], addressLine1: e.target.value };
            ...
            <div className="day-header">
              ...
              <label className="toggle-switch">
                <input
                  ...
                  onChange={(e) => updateDayTiming(day, "isOpen", e.target.checked)}
                />
            ...
            {timing.isOpen && (
              <div className="time-inputs">
                ...
                <input
                  type="time"
                  value={timing.close}
                  onChange={(e) => updateDayTiming(day, "close", e.target.value)}
                  className="time-input"
                />
              </div>
            )}
...
const DocumentVerificationStep = ({ onSubmit, formData, setFormData, config }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const uploadedRequiredDocs = documents.filter(doc => 
      doc.file && doc.docType && doc.number && 
      requiredDocTypes.includes(doc.docType)
    ).length;
    
    if (config.documentVerification.isRequired && uploadedRequiredDocs < requiredDocsCount) {
      showErrorToast(`Please upload all ${requiredDocsCount} required documents`);
      return;
    }
  ...
  const handleDocChange = (index, field, value) => {
    ...
    if (field === "docType" && value) {
      const docTypeConfig = config.documentVerification.documentTypes
        .find(dt => dt.name === value);
  ...
  const handleFileChange = (index, file) => {
    ...
    if (docTypeConfig && docTypeConfig.acceptedFormats) {
      ...
      if (!docTypeConfig.acceptedFormats.includes(fileExtension)) {
        showErrorToast(`File format must be one of: ${docTypeConfig.acceptedFormats.join(', ')}`);
        return;
      }
    }
            ...
            <div className="form-group">
              ...
              >
                ...
                {allDocTypes.map((docType) => {
                  const docConfig = config.documentVerification.documentTypes
                    .find(dt => dt.name === docType);
                  return (
              ...
              <>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    accept={(() => {
                      const docConfig = config.documentVerification.documentTypes
                        .find(dt => dt.name === doc.docType);
                      return docConfig?.acceptedFormats
                        ? docConfig.acceptedFormats.map(f => `.${f}`).join(',')
                        : "image/*,.pdf";
                    })()}
